...
DWORD WINAPI sa_Length(const SECURITY_ATTRIBUTES* sa) {
   if ((uint)sa < MIN_VALID_POINTER) return(debug("invalid parameter sa = 0x%p (not a valid pointer)", sa));
   return(sa->nLength);
   #pragma EXPORT
}
...
LPVOID WINAPI sa_SecurityDescriptor(const SECURITY_ATTRIBUTES* sa) {
   if ((uint)sa < MIN_VALID_POINTER) return((LPVOID)debug("invalid parameter sa = 0x%p (not a valid pointer)", sa));
