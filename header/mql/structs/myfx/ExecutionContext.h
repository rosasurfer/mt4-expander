...
struct EXECUTION_CONTEXT {                         // -- offset ---- size --- description ----------------------------------------------------------------------------------------
   ...
   int                mqlError;                    //       584         4     Error-Code eines aufgetretenen MQL-Fehlers      (variabel)   => welcher MQL-Fehler ist aufgetreten
   int                dllError;                    //       588         4     Error-Code eines aufgetretenen DLL-Fehlers      (variabel)   => welcher DLL-Fehler ist aufgetreten
   char*              dllErrorMsg;                 //       592         4     Text des DLL-Fehlers                            (variabel)   => ...
   int                dllWarning;                  //       596         4     Error-Code einer aufgetretenen DLL-Warnung      (variabel)   => ...
   char*              dllWarningMsg;               //       600         4     Text der DLL-Warnung                            (variabel)   => ...
   BOOL               logging;                     //       604         4     Logstatus                                       (konstant)   => was logge ich
   char               logFile[MAX_PATH];           //       608       260     Name der Logdatei (szchar)                      (konstant)   => wohin logge ich
};                                                 // ----------------------------------------------------------------------------------------------------------------------------
                                                   //               = 868                                                                     warum bin ich nicht auf Ibiza
